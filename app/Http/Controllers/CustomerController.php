...
class CustomerController extends Controller
{
    ...
    {
        DB::beginTransaction();
        $customerCount = DB::table('customers')->max('unique_id');
        try {
            $customer = Customer::create([
                //'unique_id' => str_pad(mt_rand(0, 9999), 4, '0', STR_PAD_LEFT),
                'unique_id' => $customerCount == null ? 1 : $customerCount  + 1,
                'name' => $request->input('name'),
                'phone' => $request->input('phone'),
                'parent_customer_id' => $customerCount == null ? 1 : $customerCount  + 1, //customer_id and parent_customer_id are same beacuse, another customer belongs or not,
            ]);
